...
if __name__ == "__main__":
    ...
    for mega_rounds in range( 100 ):
        ...
        data, test_wins, length_data = evaluate_networks( actors, evaluation_prob )
        win_data.append([mega_rounds, test_wins[PLAYER_RED], test_wins[PLAYER_BLUE], test_wins[DRAW]])
    ...
    data, test_wins, length_data = evaluate_networks( actors, evaluation_prob )
    win_data.append([100, test_wins[PLAYER_RED], test_wins[PLAYER_BLUE], test_wins[DRAW]])

    win_data = np.array(win_data)
    print(win_data)

    plt.scatter(win_data[:, 0], win_data[:, 1], color='red')
    plt.scatter(win_data[:, 0], win_data[:, 2], color='blue')
    plt.scatter(win_data[:, 0], win_data[:, 3], color='yellow')
    plt.show()
