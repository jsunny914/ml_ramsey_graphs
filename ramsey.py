...
def get_batch(data, size, networks, current_player):
    ...
    for i in range(size):
        current_state, action, next_state, reward, game_over = records[i]
...
def play_game(networks, data, probabilities):
    ...
    while remaining_edges > 0:
        ...
        if clique_4(adj_mat, new_edge, current_player):
            record = [ current_state, action, next_state, WIN_REWARD, True ]
            ...
            if remaining_edges == 0:
                record = [ current_state, action, next_state, DRAW_REWARD, True ]
                data[ current_player ].append( record )
                print_graph(adj_mat)
                return 0, remaining_edges
            else:
                record = [ current_state, action, next_state, MOVE_REWARD, False ]
                data[ current_player ].append( record )
                current_player *= -1        
